...
func checkIssue(url string) bool {
	...
	owner, repo, number, err := sliceUrl(url)
	if err != nil {
		return false
	}
	...
	if err != nil {
		return false
		log.Infof("Failed to get the Github issue: %v", err)
	}
...
func sliceUrl(u string) (string, string, int, error) {
	var (
		owner  string
		repo   string
		number int
	...
	if len(slice) < 6 {
		return owner, repo, number, errors.New("Failed to slice the URL")
	}
	...
	if err != nil {
		return owner, repo, number, errors.New("Cannot convert the issue to a number")
	}
